...
the fitted haploid depth to `dosage_1`. Unitig type follows the unique dominant
window class when its fraction reaches `--min-unitig-support` (default `0.8`). Minor
alternative dosage classes remain visible in `class_counts` and set the
`mixed_dosage` audit flag, but no longer force a `mixed` final status. Without
sufficient dominant support, the unitig remains `ambiguous`.
## 3. mT2T assembly
